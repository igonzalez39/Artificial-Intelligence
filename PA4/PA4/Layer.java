...
public class Layer {
	...
	public void computeOutputDelta() {
		...
		Vector target = targ.difference(act); //target values set to the difference btwn curr target and act,
		//serves as '(T.i-a.i)' part of equation above 
		
		
		
		//"scaled between the given minimum and maximum values, to each of the
        //elements of this vector"
		Vector Nt = net.derivative(min, max); // serves as g'(in.i)
        
        
        
        for (int i = 0; i < n; i++) {
            //iterates and fills delta with g'(in.i)(T.i-a.i)
            delta.set(i, target.get(i) * Nt.get(i));
        }
	...
	public void computeHiddenDelta() {
		...
		// PLACE YOUR CODE HERE ...
		 ...
		 Vector Nt; //g'(in.i)
	        Nt = net.derivative(min, max);
	        // begin mapping of layer 
	        for (Projection map : outputs) {
	            
	            Matrix trans;//store transposed matrix 
	            trans = map.W.transpose(); //consider weight when map transposed to "downstream" 
	            
	            Vector m  = trans.product(map.output.delta); //vector hold product of transposed matrix weight and delta of map
	            //(map.outputs.delta);

	            for (int i = 0; i < n; i++) {
	            	//fill delta 
	                delta.set(i, m.get(i) * Nt.get(i));
	            }
	        }
